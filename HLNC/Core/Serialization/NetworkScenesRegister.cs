...
namespace HLNC.Serialization
{
    ...
    [Tool]
    public partial class CollectedNetworkFunction : GodotObject
    {
    ...
    public static partial class NetworkScenesRegister
    {
        ...
        /// 
        /// </summary>
        /// <returns>A dictionary of scene paths to their corresponding node paths</returns>
        public static Dictionary<string, List<string>> ListScenes() {
            return SCENES_PACK.Keys.ToDictionary(scene => scene, scene => STATIC_NETWORK_NODE_PATHS_PACK[scene].Keys.ToList());
        }
        ...
        public static List<CollectedNetworkProperty> ListProperties(string scene, string node) {
            if (!PROPERTIES_MAP.ContainsKey(scene) || !PROPERTIES_MAP[scene].ContainsKey(node)) {
                return [];
            }
            return PROPERTIES_MAP[scene][node].Values.ToList();
        }
