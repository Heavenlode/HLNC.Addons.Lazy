...
namespace HLNC
{
	/**
		...
		On every network tick, all NetworkNode nodes in the scene tree automatically have their <see cref="NetworkProperty">network properties</see> updated with the latest data from the server.
	...
	*/
	public partial class NetworkNode : Node, INetworkNode, INotifyPropertyChanged, INetworkSerializable<NetworkNode>, IBsonSerializable<NetworkNode>
	{
		...
		public static HLBuffer NetworkSerialize(WorldRunner currentWorld, NetPeer peer, NetworkNode obj)
		{
		...
		public static NetworkNode NetworkDeserialize(WorldRunner currentWorld, HLBuffer buffer, NetworkNode initialObject)
		{
			...
			var node = currentWorld.GetNodeFromNetworkId(networkID).Node as NetworkNode;
			if (staticChildId > 0)
			{
				node = node.GetNodeOrNull(NetworkScenesRegister.UnpackNode(node.SceneFilePath, staticChildId)) as NetworkNode;
			}
			return node;
		}
		...
		public static NetworkNode BsonDeserialize(Variant context, byte[] bson, NetworkNode obj)
		{
			var data = BsonSerializer.Deserialize<BsonDocument>(bson);
			if (data.IsBsonNull) return null;
			var doc = data.AsBsonDocument;
			var node = obj == null ? new NetworkNode() : obj;
