...
namespace HLNC
{
    ...
    public partial class NetworkRunner : Node
    {
        ...
        public void StartServer()
        {
            ...
            if (err != Error.Ok)
            {
                Debugger.Log($"Error starting: {err}", Debugger.DebugLevel.ERROR);
                return;
            }
            ...
            if (err != Error.Ok)
            {
                Debugger.Log($"Error starting debug server: {err}", Debugger.DebugLevel.ERROR);
                return;
            }
            debugEnet.Compress(ENetConnection.CompressionMode.RangeCoder);
            Debugger.Log($"Started debug server on {ServerAddress}:{DebugPort}", Debugger.DebugLevel.VERBOSE);
        }
