    ...
    public partial class WorldRunner : Node
    {
        ...
        public override void _Ready()
        {
            ...
            if (NetworkRunner.Instance.IsServer)
            {
                ...
                int port = random.Next(59910, 63001);
                int attempts = 0;
                const int MAX_ATTEMPTS = 1000;
                do
                {
                    err = DebugEnet.CreateHostBound(NetworkRunner.Instance.ServerAddress, port, NetworkRunner.Instance.MaxPeers);
        ...
        public async void ServerProcessTick()
        {
            ...
            foreach (var net_id in NetworkScenes.Keys)
            {
                ...
                foreach (var networkChild in networkNode.StaticNetworkChildren)
                {
                    ...
                    {
                        Log($"Network child node is unexpectedly null: {networkNode.Node.SceneFilePath}", Debugger.DebugLevel.ERROR);
                    }
            ...
            foreach (var peer in peers)
            {
                ...
                {
                    Log($"Data size {size} exceeds MTU {NetworkRunner.MTU}", Debugger.DebugLevel.WARN);
                }
                ...
                if (DebugEnet != null)
                {
                    ...
                    HLBytes.PackVariant(debugBuffer, PeerStates[peer].Id, packType: true);
            ...
            var fullGameState = DataTransformer.ToBSONDocument(RootScene.Node as INetworkNode, recurse: true);
        ...
        {
            NetworkRunner.Instance.PeerWorldMap.Remove(peer);
        ...
        internal void ReceiveInput(NetPeer peer, HLBuffer buffer)
        {
            ...
            if (node == null)
            {
                Log($"Received input for unknown node {worldNetworkId}", Debugger.DebugLevel.ERROR);
                return;
            }

            if (node.InputAuthority != peer)
            {
                Log($"Received input for node {worldNetworkId} from unauthorized peer {peer}", Debugger.DebugLevel.ERROR);
                return;
            }
